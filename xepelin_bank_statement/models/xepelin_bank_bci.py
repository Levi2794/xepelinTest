...
class BankBci(models.Model):
    _name = "xepelin.bank.bci"
    _description = "BCI Bank statements"
    ...
    def create(self, vals):
        ...
        result = super(BankBci, self).create(vals)
        return result

    def write(self, vals):
        result = super(BankBci, self).write(vals)
        return result

    def action_redirect(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "xepelin_bank_statement.xepelin_bank_bci_action"
        )
        action["target"] = "main"
        return {
