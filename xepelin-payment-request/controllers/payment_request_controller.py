...
class PaymentRequestController(http.Controller):
    @http.route("/payment-request", type='json', auth="public", methods=['POST'], csrf=False)
    def post_paymnet_request(self):
      ...
      try:
        ...
        if len(beneficiary_name) > BENEFICIARY_NAME_LENGTH:
          message = 'The name of the beneficiary exceeds 40 characters'
        ...
        if country_code == 'CL':
          bank = request.env['xepelinpaymentrequest.bank'].sudo().search(
            [('name', '=', bank_name), ('country_id.code', '=', country_code)],
            limit=1
          )
          if bank.code == False:
        ...
        area = _get_dynamic_key_from_input(request.jsonrequest, 'area')
        invoice_link = _get_dynamic_key_from_input(request.jsonrequest, 'invoiceLink')
        invoice_date = _get_dynamic_key_from_input(request.jsonrequest, 'invoiceDate')
        if invoice_date != None:
          invoice_date = datetime.datetime.strptime(invoice_date, '%d/%m/%Y')
        ...
        request.env['xepelinpaymentrequest.paymentrequest'].sudo().create({
